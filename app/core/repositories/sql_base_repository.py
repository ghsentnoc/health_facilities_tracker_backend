...
class BaseReadRepository(BaseRepository[T]):
    ...
    def get_multiple_by_ids(self, *, entity_ids: list[str]) -> list[T]:
        """Get multiple entities by their IDs.

        Args:
            entity_ids (list[str]): The list of entity IDs.

        Returns:
            list[T]: A list of entity instances.
        """
        return self.db_session.query(self.model).filter(self.model.id.in_(entity_ids)).all()  # type: ignore
    ...
    def count_by_field(self, *, field_name: str, value: Any, operator: str = "eq") -> int:
        """Count entities by a specific field.
        ...
        Returns:
            int: The count of entities matching the criteria.
        """
        ...
        query = self.db_session.query(self.model).filter(operator_func(field, value))  # type: ignore
    ...
    def exist_but_deleted(self, *, field_name: str, value: Any, operator: str = "eq") -> bool:  # type: ignore
        ...
        if entity:  # type: ignore
            ...
            return bool(entity.is_deleted)  # type: ignore
        return False
...
class BaseWriteRepository(BaseRepository[T]):
    ...
    def save(self, *, object_to_save: T) -> T:
        ...
        try:
            ...
            if not self.db_session.info.get("managed_transaction", False):
