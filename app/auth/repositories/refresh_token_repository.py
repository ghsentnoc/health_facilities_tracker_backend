from typing import Any, Optional, Type, Union

from sqlalchemy.orm import Session

from app.auth.models import RefreshToken
from app.core.repositories.sql_base_repository import BaseReadRepository, BaseWriteRepository


class RefreshTokenRepository(BaseReadRepository[RefreshToken], BaseWriteRepository[RefreshToken]):
    """Repository for managing RefreshToken entities."""

    def get_all(
        self,
        *,
        filters_without_joins: list,
        filters_with_joins: Optional[list] = None,
        pagination: Optional[dict[str, int]] = None,
        filters: Optional[dict[str, Any]] = None,
        sort: Optional[dict[str, str]] = None,
        jurisdiction_zone: Optional[list[str]] = None,
    ) -> Union[list[RefreshToken], list[Type[RefreshToken]]]:
        """Get all regions.

            filters_without_joins (list): Filters without no joins
            filters_with_joins (list): Filters with joins
            pagination (dict[str, int]): Pagination parameters.
            filters (dict[str, Any]): Filter parameters.
            sort (dict[str, str]): Sort parameters.
            jurisdiction_zone (list[str], optional): Jurisdiction zone filter.

        Returns:
            list[RefreshToken]: A list of all entity instances.
        """
        query = self.db_session.query(RefreshToken)

        return self._default_get_all(
            filters_without_joins=filters_without_joins,
            filters_with_joins=filters_with_joins,
            filters=filters,
            sort=sort,
            pagination=pagination,
            query=query,
        )

    def __init__(self, *, db_session: Session, model: type[RefreshToken] = RefreshToken) -> None:
        """Initialize the RefreshTokenRepository with a database session and model.

        Args:
            db_session (Session): The SQLAlchemy database session.
            model (User): The RefreshToken model class.
        """
        self.db_session = db_session
        self.model = model
        super().__init__(db_session=db_session, model=model)

    def create(self, *, data: dict) -> RefreshToken:
        """The method to create a new refresh_token entity.

        Args:
            data (dict): The refresh_token data needed to create the entity.

        Returns:
            T: The newly created refresh_token.
        """
        return self._default_create(data=data)
